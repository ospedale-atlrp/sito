...
(function () {
  ...
  function injectStyleOnce() {
    ...
    style.textContent = `
      ...
      .pm-chat-overlay.open .pm-chat-window { transform:scaleY(1) scaleX(1); opacity:1; }
      ...
      .pm-chat-loading { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px;
        padding:50px 20px; opacity:0.55; font-size:0.85rem; }
      .pm-chat-spinner { width:22px; height:22px; border-radius:50%; border:2.5px solid rgba(127,127,127,0.3);
      ...
      .pm-chat-form textarea:disabled { opacity:0.6; }
  ...
  async function pmOpenReservationChat(reservationId, opts) {
    ...
    overlay.innerHTML =
      '<div class="pm-chat-window">' +
        ...
        '</div>' +
        '<div class="pm-chat-messages" id="pm-chat-messages"><div class="pm-chat-loading"><span class="pm-chat-spinner"></span>' + esc(t('common.loading', 'Caricamento...')) + '</div></div>' +
        (readOnly ? '' :
    ...
    if (!readOnly && form && input) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (messagesBox.querySelector('.pm-chat-empty') || messagesBox.querySelector('.pm-chat-loading')) {
